...
class DiscourseShop::Product < ActiveRecord::Base
  ...
  def image_url; self[:image_url]; end

  after_create  { ::DiscourseShop::Log.log!("[Product] CREATE id=#{id}") }
  after_update  { ::DiscourseShop::Log.log!("[Product] UPDATE id=#{id}") }
  after_destroy { ::DiscourseShop::Log.log!("[Product] DESTROY id=#{id}") }
end
