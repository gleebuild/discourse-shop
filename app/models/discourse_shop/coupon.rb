...
class DiscourseShop::Coupon < ActiveRecord::Base
  ...
  after_create  { ::DiscourseShop::Log.log!("[Coupon] CREATE id=#{id}") }
  after_update  { ::DiscourseShop::Log.log!("[Coupon] UPDATE id=#{id}") }
  after_destroy { ::DiscourseShop::Log.log!("[Coupon] DESTROY id=#{id}") }
end
