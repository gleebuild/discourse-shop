...
class DiscourseShop::Order < ActiveRecord::Base
  ...
  after_create  { ::DiscourseShop::Log.log!("[Order] CREATE id=#{id}") }
  after_update  { ::DiscourseShop::Log.log!("[Order] UPDATE id=#{id}") }
  after_destroy { ::DiscourseShop::Log.log!("[Order] DESTROY id=#{id}") }
end
