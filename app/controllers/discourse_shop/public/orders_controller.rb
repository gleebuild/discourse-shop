...
class DiscourseShop::Public::OrdersController < DiscourseShop::Public::BaseController
  ...
  def create
    ...
    quantity = params[:quantity].to_i; quantity = 1 if quantity <= 0
    order = DiscourseShop::Order.new(
      user: current_user, product: product, variant: variant, quantity: quantity,
      currency: product.currency, status: :pending,
      recipient_name: params[:shipping][:name], phone: params[:shipping][:phone],
      country: params[:shipping][:country], province: params[:shipping][:province],
      city: params[:shipping][:city], address: params[:shipping][:address],
    ...
    if (code = params[:coupon_code]).present?
      ...
      order.apply_coupon!(coupon) if coupon
    end
    order.save!
