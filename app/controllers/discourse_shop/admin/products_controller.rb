...
class DiscourseShop::Admin::ProductsController < DiscourseShop::Admin::BaseController
  ...
  def create
    p = DiscourseShop::Product.new(product_params); p.status = :draft if p.status.nil?; p.save!
    render_json_dump(product: DiscourseShop::ProductSerializer.new(p))
  end
  def update
    p = DiscourseShop::Product.find(params[:id]); p.update!(product_params)
    render_json_dump(product: DiscourseShop::ProductSerializer.new(p))
  end
  def destroy
    DiscourseShop::Product.find(params[:id]).destroy!; render_json_dump(success: true)
  end
  def publish
    p = DiscourseShop::Product.find(params[:id]); p.update!(status: :active); render_json_dump(success: true)
  end
  def unpublish
    p = DiscourseShop::Product.find(params[:id]); p.update!(status: :draft); render_json_dump(success: true)
  end
  private
  def product_params
